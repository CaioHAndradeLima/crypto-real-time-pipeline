...
.PHONY: \
	...
	infra.start-postgres infra.start-airbyte infra.configure-airbyte infra.start-airflow infra.up infra.down \
	create-env provision-snowflake configure-dbt-profile setup-local-environment \
	start-postgres start-airbyte configure-airbyte start-airflow start-local-infra run-local-environment \
