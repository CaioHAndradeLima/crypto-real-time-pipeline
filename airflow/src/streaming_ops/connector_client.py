...
class KafkaConnectClient:
    ...
    def upsert_snowflake_sink_connector(self) -> None:
        ...
        with urlopen(req, timeout=30) as response:
            if response.status // 100 != 2:
                raise RuntimeError(
                    f"Connector configure failed with status {response.status}"
                )
    ...
    def assert_connector_running(self) -> None:
        ...
        if connector_state != "RUNNING":
            raise RuntimeError(
                f"Connector state is {connector_state}, expected RUNNING"
            )
    ...
    def get_status(self) -> dict:
        status_url = (
            f"{self.config.connect_url}/connectors/{self.config.connector_name}/status"
        )
    ...
    def delete_connector(self) -> None:
        ...
        with urlopen(req, timeout=15) as response:
            if response.status not in (200, 202, 204):
                raise RuntimeError(
                    f"Connector delete failed with status {response.status}"
                )
